...
path_predictions='./bart_model/outputs/'+nameDataset+'.csv'
if not os.path.exists(path_predictions):
    ...
    gcc.download_blob(BUCKET_NAME,'tfg/summaries/BART/'+nameDataset+'.csv',path_predictions)
else:
    remove_predictions = False
    print("File with predictions is on notebook")
...
path_scores='./bart_model/evaluation/'+nameDataset+'.txt'
...
gcc.upload_blob(BUCKET_NAME,path_scores,'tfg/scores/BART/'+nameDataset+'-score.txt' )
