...
path_predictions='./led_inference/summaries/'+nameDataset+'.csv'
if not os.path.exists(path_predictions):
    ...
    gcc.download_blob(BUCKET_NAME,'tfg/summaries/LED/'+nameDataset+'.csv',path_predictions)
else:
    remove_predictions = False
    print("The model exists on the notebook")
...
path_scores='./led_inference/evaluation/'+nameDataset+'.txt'
...
gcc.upload_blob(BUCKET_NAME,path_scores,'tfg/scores/LED/'+nameDataset+'-score.txt' )
